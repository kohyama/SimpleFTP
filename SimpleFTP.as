package  {
	...
	public class SimpleFTP {
		public static function putFile(host:String, user:String, pass:String, 
									   path:String, contents:String, listener:Function) {
		...
		private function putFile(path:String, contents:String, listener:Function):void
		{
			this.path = path;
		...
		private function write(mes:String)
		{
			ctrlSocket.writeUTFBytes(mes + "\r\n");
			ctrlSocket.flush();
		}
		
		private function response(res:String, contents:String = null) {
			step = 11;
			if (put)
				listener(res);
			else
